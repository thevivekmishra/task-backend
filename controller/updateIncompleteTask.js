...
export const updateIncompleteTask = async (req, res, next) => {
    try {
        ...
        res.status(200).json({
            message: "Task updated to incomplete successfully",
            task
        });
    } catch (error) {
        console.error("Failed to update task incompletion status", error);
        res.status(500).json({
